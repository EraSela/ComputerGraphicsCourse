...
const roadMat = new THREE.MeshStandardMaterial({map:roadTexture, roughness: 0.9 });
...
gltfLoader.load(
  ...
  (gltf) => {
    ...
    flowerPot1.scale.set(12, 3, 2);
    flowerPot1.position.set(18.78, 0.1, -12.75); 

    flowerPot1.traverse((child) => {
      if (child.isMesh) {
    ...
    flowerPot2.position.set(18.78, 0.1, 2.20); 
...
const blueTexture = textureLoader.load('textures/blue_building.jpg'); 
blueTexture.wrapS = THREE.RepeatWrapping;
blueTexture.wrapT = THREE.RepeatWrapping;
blueTexture.repeat.set(1, 1); 
...
// Center position of the building
const blue1Center = new THREE.Vector3(-80, buildingHeight/2, 28);

// Back wall (thin)
const backWall = new THREE.Mesh(
  new THREE.BoxGeometry(buildingWidth, buildingHeight, 0.5),
  blueMat
);
backWall.position.set(blue1Center.x, blue1Center.y, blue1Center.z - buildingDepth/2 + 0.25);
backWall.castShadow = true;
scene.add(backWall);

// Side walls
const sideWallLeft = new THREE.Mesh(
  new THREE.BoxGeometry(0.5, buildingHeight, buildingDepth),
  blueMat
);
sideWallLeft.position.set(blue1Center.x - buildingWidth/2 + 0.25, blue1Center.y, blue1Center.z);
...
sideWallRight.position.set(blue1Center.x + buildingWidth/2 - 0.25, blue1Center.y, blue1Center.z);
...
topWall.position.set(blue1Center.x, buildingHeight - 0.25, blue1Center.z);
scene.add(topWall);
...
bottomWall.position.set(blue1Center.x, 0.25, blue1Center.z);
scene.add(bottomWall);

// Glass front
const frontGlass = new THREE.Mesh(
  new THREE.BoxGeometry(buildingWidth, buildingHeight - 1, 0.5),
  blueMat
);
frontGlass.position.set(blue1Center.x, blue1Center.y, blue1Center.z + buildingDepth/2 - 0.25);
...
interior.position.set(blue1Center.x, 2.5, blue1Center.z + 5); 
scene.add(interior);

//  BLUE BUILDING 2 
const blue2Center = new THREE.Vector3(-80, buildingHeight/2, -10);

const blue2 = new THREE.Mesh(
  new THREE.BoxGeometry(buildingWidth, buildingHeight, buildingDepth),
  blueMat
);
blue2.position.copy(blue2Center);
...
const yellowTexture = textureLoader.load('textures/yellow_building.jpg'); 
yellowTexture.wrapS = THREE.RepeatWrapping;
yellowTexture.wrapT = THREE.RepeatWrapping;
yellowTexture.repeat.set(1, 1); 

const yellowMat = new THREE.MeshStandardMaterial({ map:yellowTexture, roughness: 0.4 });
...
const lightBlueTexture = textureLoader.load('textures/light_blue_building.jpg'); 
lightBlueTexture.wrapS = THREE.RepeatWrapping;
lightBlueTexture.wrapT = THREE.RepeatWrapping;
lightBlueTexture.repeat.set(1, 1); 
const lightBlueMat = new THREE.MeshPhongMaterial({ map:lightBlueTexture, shininess: 80 });
...
// hanging out area
const grayAreaMat = new THREE.MeshLambertMaterial({ map:roadTexture });
...
const treeTrunkMat = new THREE.MeshStandardMaterial({ map:barkTexture });
...
function animate() {
  ...
  light1.intensity = 80 + Math.sin(time * 2) * 20;
  light2.intensity = 80 + Math.sin(time * 2 + 1) * 20;
  light3.intensity = 80 + Math.sin(time * 2 + 2) * 20;
  light4.intensity = 80 + Math.sin(time * 2 + 3) * 20;
  
  spotLight.target.position.x = Math.sin(time) * 5;
  spotLight.target.updateMatrixWorld();
  
  spotLight1.intensity = 1500 + Math.sin(time * 2 + 1) * 300;
  spotLight1.target.position.z = -40 + Math.cos(time) * 5;
...
window.addEventListener('keydown', (e) => {
  if (e.key === 'l') {
    lightsOn = !lightsOn;
    light1.visible = lightsOn;
    light2.visible = lightsOn;
    light3.visible = lightsOn;
    light4.visible = lightsOn;
    light5.visible = lightsOn;
    light6.visible = lightsOn;
    light7.visible = lightsOn;
    light8.visible = lightsOn;
  }
});
